...
CREATE OR ALTER PROCEDURE dbo.DemoJobStepDuration
	@MINIMUM_MINUTES_DURATION	INT = 1
,	@WAIT						BIT = 1
AS
BEGIN
	DECLARE
		@MINIMUM_SECONDS	INT = @MINIMUM_MINUTES_DURATION * 60
	,	@RANDOM_SECONDS		INT
	,	@WAIT_UNTIL			DATETIME
	,	@TIME				VARCHAR(50);

	SET @RANDOM_SECONDS = CAST(RAND() * 250 AS INT);
	SET @WAIT_UNTIL = 
		DATEADD(
			second
		,	(@MINIMUM_SECONDS + @RANDOM_SECONDS)
		,	GETDATE()
	);
...
BEGIN
	...
	SET @SNAPSHOT_KEY = NEXT VALUE FOR dbo.SnapshotKey;
